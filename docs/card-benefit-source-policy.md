...
`GEMINI_API_KEY`가 있으면 `CARD_BENEFIT_AI_MODEL`의 JSON schema 출력으로 전체 카드 혜택 후보를 생성한다. 키가 없거나 호출이 실패하면 대표 카드 전용 보수적 규칙 추출기를 사용한다. AI 출력은 다음 조건을 모두 통과해야 검수자가 승인할 수 있다.
...
첫 상품 페이지에는 혜택 적용 기간 필드가 없으므로 기간형 공지 adapter를 추가할 때 시작일·종료일과 문서 공시일 검증을 별도로 추가한다.
