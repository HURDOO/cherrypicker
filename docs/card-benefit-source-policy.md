...
`OPENAI_API_KEY`가 있으면 기본 `gpt-5.6-luna`를 OpenAI Responses API로 호출한다. 첫 호출은 공식 문서의 상시 혜택·유효 프로모션·실적·한도·제외 조건을 혜택 section 인벤토리로 전수 목록화하고, 두 번째 호출은 이를 중첩 JSON Schema의 `CardBenefitExtraction`과 section별 rule coverage로 직접 구조화한다. 고정 필드로 충분한 규칙은 `program=null`, 충분하지 않은 규칙은 허용 목록 DSL v1 program과 `FLAT 0` fallback을 출력한다. 서버는 기존 target·condition·limitConfig의 legacy DSL을 안전한 기본값으로 합성하며 개별 서비스 기간 한도와 카드 통합 한도를 분리한다. DSL로도 표현할 수 없는 문구는 근거·영향 rule ID·계산값 영향 여부를 `unsupportedClauses`에 반환한다. 두 결과 모두 원문 URL·인용 문장·PDF 페이지를 애플리케이션 validator가 다시 확인한다. 인벤토리 section이 규칙이나 근거에 연결되지 않으면 후보 승인을 차단한다. 키가 없거나 호출·파싱이 실패하면 SOL트래블만 기존 보수적 규칙 추출기를 사용하고, 범용 AI 우선 adapter는 불완전한 후보를 만들지 않고 수집 실패로 남긴다.
...
- 존재하는 카테고리와 브랜드만 참조
- DSL language version, 연산자·입력·필드 allowlist, 타입, 참조, 노드 256개·깊이 16·이력 5,000건 상한과 비정상 숫자를 검증
- DSL program이 있는 규칙은 `program`과 모든 AST `programPaths`를 공식 인용에 연결하고 숫자 literal도 해당 인용에서 확인하며, 다른 coverage 규칙을 대신 가리키는 근거와 금액·조건·한도에 영향을 주는 `unsupportedClauses`가 하나라도 남으면 게시 차단
- 카드별 실적 제외 정책은 `card.performancePolicy`의 허용된 조건과 규칙별 공식 URL·원문 인용·PDF 페이지로 구조화한다. 공식 `전월 이용실적 제외 대상` 절이 있는데 정책도 금액 영향 미지원 문구도 없으면 게시를 차단한다. 기본은 승인금액 전액의 예상 실적 반영이며, 카드별 공식 문구에 명시된 제외 규칙이 해당 거래에서 판정될 때만 제외한다. 카드 할인 적용 거래 전체 제외는 거래 전체 실적 0원으로 처리하고, 할인 적용 여부가 판정 중인 경우만 0원·불확실로 보수 처리한다. 무이자할부·상품권처럼 거래 종류를 식별할 수 없는 예외는 정책만 추가해 자동 제외가 완성된 것으로 취급하지 않고 금액 영향 미지원 문구와 게시 게이트를 유지한다. 정책 변경은 고위험 diff로 검수하며 다른 카드의 제외 규칙을 재사용하지 않는다.
- 공식 문구가 실제 식별 가능한 결제처를 가리키고 registry에 없으면 관리자 create-only 경로로 등록한 뒤 재검증한다. 경기·상품·장소 등 구매 상황은 가상 결제처로 등록하지 않고 `purchaseScenario`의 표시명·별칭·확인 질문을 공식 인용에 연결해 검수한다. 상황 선택 전이나 질문 확인 전에는 확정 혜택이 아니며, 의미가 모호한 후보는 게시하지 않는다. 특정 결제처·상황 혜택을 카테고리 전체로 확대하거나 AI가 새 브랜드 ID를 임의 생성한 후보는 게시하지 않는다.
