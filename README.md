...
- 공식 목록에서 사라진 게시 혜택은 삭제 의심 후보로 전환하고, 관리자의 `사라짐 확정·만료` 또는 `오탐·혜택 유지` 선택으로 처리
...
카드 전체 확인 결과는 `card_benefit_collection_runs`에 카드별 상태, AI 구조화 수, 검증 오류와 출처 실패를 보존합니다. 관리자 카드 혜택 화면은 최근 20회 실행과 두 번 이상 연속 문제가 발생한 카드를 표시합니다. 애플리케이션 프로세스 내부 타이머는 다중 프로세스·재시작 시 중복 실행될 수 있으므로 사용하지 않으며, 실제 실행 주기는 배포 환경의 단일 외부 스케줄러에서 관리합니다.
